...
2. **SCS environment:** `platform/kubernetes/environments/scs/default.nix` — cluster config
   (ingress=haproxy, storage=ceph-rbd/ceph-cephfs, registry=registry.infra.local:5001 local mirror)
...
2. **Image registry**: Use `env.registry.url` (registry.infra.local:5001) for local images.
