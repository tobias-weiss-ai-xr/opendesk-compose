...
for vol in pgdata casdoor_data opencloud_data synapse_data notes_data sogo_data stalwart_data traefik-acme redis_data paperless_data paperless_media paperless_export invoiceninja_data invoiceninja_public invoiceninja_storage cryptpad_data cryptpad_blob; do
  echo "  Restoring $vol..."
  if [ -d "$vol" ]; then
